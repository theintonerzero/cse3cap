...
- [x] `tokens.css`: colour, spacing, radius as CSS variables; light and dark via
      `data-theme`. No raw hex anywhere else in the codebase. CAP-1 (#15), with
      `scripts/check-tokens.sh` in CI holding the no-raw-hex rule in place
- [x] Core components: Card, Button, Chip, Badge, TextArea (debounced), ProgressBar,
      ...
      All ten, across CAP-3 (#18), CAP-4 (#16, #22) and CAP-6 (#20). `web/gallery.html`
      renders every one of them in its states
...
- [ ] App shell: router, token context, role-aware nav from `/auth/me`. CAP-5 is in
      review as #30; until it merges every screen below has no route to mount on
...
*Assessor*
- [x] Review queue: worklist with per-reflection progress. CAP-10 (#19), all four
      states. It mounts through its own `review-queue.html` entry because CAP-5 did not
      exist when it was built; moving it onto the router is a follow-up that unblocks
      with #30
...
- [x] Policies covering every permission-matrix row, resolved from `gig_participants`.
      `GigPolicy`, `ReflectionPolicy`, `FrameworkPolicy` and `ExportPolicy`, twelve methods
      between them, with authorisation moved out of the controllers. CAP-19 (#17).
      Not-yours returns 404 rather than 403, deliberately. The row-by-row audit against
      `docs/api-reference.html` has not been done independently: #17 merged without the
      review that would have walked it
...
- [~] Security review on every PR touching scoring, submit, or framework mutation. Done
      twice. On the backend PR, which touches all three: it found a counter-score accepted
      after a reflection was assessed, a gig able to hold two rubrics, and three analytics
      endpoints returning a 500 outside the error envelope. ADRs #33, #34, #35. Then on the
      frontend and API seam, CAP-24, recorded in `docs/Security-Review.md`: five findings,
      of which F1 was a live bearer token in the production bundle, fixed in #28 and held
      by `scripts/check-bundle-secrets.sh`. CAP-24's injection half is deferred until there
      is a screen rendering user text. Stays open because the commitment is per PR
