...
if (Get-Command php -ErrorAction SilentlyContinue) {
    ...
    foreach ($ext in @('pdo_mysql','mbstring','openssl','fileinfo')) {
        if (-not (& php -m | Select-String -Quiet "^$ext$")) {
...
if (Test-Path 'api') {
    if (Test-Path 'api/.env') {
        ...
        if ($AppPw) {
            # Read and rewrite rather than regex replace: a password can
            # contain characters that mean something to the regex engine.
            $lines = Get-Content 'api/.env' | ForEach-Object {
                if ($_ -like 'DB_PASSWORD=*') { "DB_PASSWORD=$AppPw" } else { $_ }
            }
            Set-Content 'api/.env' $lines -Encoding UTF8
    ...
    if (Test-Path 'api/composer.json') {
        ...
        Push-Location api; & composer install --no-interaction; Pop-Location
    }
} else {
...
if (Test-Path 'web') {
    if (-not (Test-Path 'web/.env')) {
        Set-Content 'web/.env' 'VITE_API_BASE_URL=http://localhost:8000/api/v1' -Encoding UTF8
        Ok "wrote web/.env"
...
# variable is the whole of the agent setup.
Say "Claude Code"
...
$tcp = Test-NetConnection -ComputerName $DbHost -Port 3306 -WarningAction SilentlyContinue
if ($tcp.TcpTestSucceeded) { Ok "$DbHost`:3306 reachable" }
else { Warn "Cannot reach $DbHost`:3306. Check your network, then ask in the channel." }
